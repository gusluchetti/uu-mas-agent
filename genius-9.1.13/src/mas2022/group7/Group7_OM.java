...
public class Group7_OM extends OpponentModel {
    ...
    public void init(NegotiationSession negotiationSession, Map<String, Double> parameters) {
        ...
        previousBidUtility = 1D;
        weightHypotheses = new ArrayList<>();
        ...
        for (int i = 0; i < utilitySpace.getNrOfEvaluators(); ++i) {
            ...
            switch (utilitySpace.getEvaluator(utilitySpace.getIssue(i).getNumber()).getType()) {
                case REAL: {
                    ...
                    /* Triangular */
                    int totalTriangularFunctions = 4;
    ...
    public void updateModel(Bid opponentBid, double time) {
        biddingHistory = this.negotiationSession.getOpponentBidHistory().getHistory();
        ...
        try {
            if (opponentUtilitySpace != null) { // fix we applied
    ...
    protected void findMinMaxUtility() throws Exception {
        maxUtility = getExtremeUtility(PerfectIAMhagglerBayesianModel.Extreme.MAX);
        minUtility = getExtremeUtility(PerfectIAMhagglerBayesianModel.Extreme.MIN);
    }
    ...
    private double getExtremeUtility(PerfectIAMhagglerBayesianModel.Extreme extreme) {
        double u = 0;
        for (int i = 0; i < domain.getIssues().size(); i++) {
    ...
    private double getExtremeEvaluationValue(int number, PerfectIAMhagglerBayesianModel.Extreme extreme) {
    ...
    public double getExtremeEvaluation(Evaluator evaluator, PerfectIAMhagglerBayesianModel.Extreme extreme) {
    ...
    private double initExtreme(PerfectIAMhagglerBayesianModel.Extreme extreme) {
        switch (extreme) {
            case MAX:
    ...
    private void updateEvaluationFunctions() throws Exception {
        ...
        Bid bid = biddingHistory.get(biddingHistory.size() - 1).getBid();
    ...
    public String getName() {
        return "2022 - Agent 007 'IAMhaggler Bayesian Model'";
    }
    ...
    public List<Double> getWeights(List<Issue> issues) {
        List<Double> weights = new ArrayList<>();
        for (Issue issue : issues) {
            double weight = this.getExpectedWeight(issue.getNumber());
            if (Double.isNaN(weight)) {
                System.out.println("NaN happened! :(");
                System.out.println(this.negotiationSession.getTime());
                System.out.println(this.negotiationSession);
            }
            weights.add(weight);
        }
        return weights;
